...
- 首页按“收集 → 行动 → 知识 → 计划 → 探索 → 整理”组织，不再把所有功能平铺在一个首屏里。
- “记下想法”“添加任务”“安排日程”会打开对应 Markdown 模板；任务区默认只显示未完成事项，近期安排默认只显示今天及之后的未完成日程。
- 搜索、类型筛选和标签会同步筛选知识卡片；普通知识卡链接到仓库 Markdown，来源卡优先链接到原始来源。
- 完整月历、想法关系和标签放在次级区域，需要计划或回顾时再使用。
- 日历可添加本地日程，也可导出 `.ics` 文件用于同步到外部日历。
- “本次隐藏”只会从当前浏览器视图移除知识卡片；刷新页面即可恢复，公网文件不会被网页直接删除。
- 编辑与本地上传默认折叠，展开后会生成 Markdown，可以复制、下载，也可以加入本次临时预览。
